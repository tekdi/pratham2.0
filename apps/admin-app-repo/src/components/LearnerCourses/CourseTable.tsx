...
const CourseTable: React.FC = () => {
    ...
    const fetchCourses = async () => {
      ...
      for (const userId of userIds) {
        ...
        userMap[userDetail.userData.userId] = {
          ...
          name: userDetail?.userData?.firstName + (userDetail?.userData?.lastName ? ' ' + userDetail?.userData?.lastName : ''),
  ...
  return (
   ...
   {t('CERTIFICATES.REISSUE_CERTIFICATE_ALERT')}
    ...
    >
      <Box sx={{ mb: 1 }}>
        <Typography variant="body2"><strong>{t('CERTIFICATES.FIRST_NAME')}</strong> {selectedRowData.firstName || "-"}</Typography>
      </Box>
      <Box sx={{ mb: 1 }}>
        <Typography variant="body2"><strong>{t('CERTIFICATES.MIDDLE_NAME')}</strong> {selectedRowData.middleName || "-"}</Typography>
      </Box>
      <Box>
        <Typography variant="body2"><strong>{t('CERTIFICATES.LAST_NAME')}</strong> {selectedRowData.lastName || "-"}</Typography>
      </Box>
      <Box>
        <Typography variant="body2"><strong>{t('CERTIFICATES.COURSE_NAME')}</strong> {selectedRowData.courseName || "-"}</Typography>
      </Box>
    </Paper>
