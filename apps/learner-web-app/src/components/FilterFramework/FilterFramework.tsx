...
const FilterFramework: React.FC<FilterFrameworkProps> = ({
  ...
  const shouldShowFilter = React.useMemo(() => {
    if (typeof window === 'undefined') return true; // SSR: default to showing
    try {
      const uiConfigStr = localStorage.getItem('uiConfig');
      if (!uiConfigStr) return true; // If uiConfig absent, show filter
      const uiConfig = JSON.parse(uiConfigStr);
      // If showFilter is explicitly false, don't show. Otherwise show (true or absent)
      return uiConfig.showFilter !== false;
    } catch (error) {
      console.error('Error reading uiConfig from localStorage:', error);
  ...
  const frameworkFilterCount = Object.keys(selectedFilters).filter(
    (categoryCode) =>
  ...
  const transformFiltersForCallback = (
    filters: FilterState,
    staticFiltersData: Record<string, string[]> = {}
    ...
    Object.keys(filters).forEach((categoryCode) => {
      ...
      if (selectedTermCodes && selectedTermCodes.length > 0) {
        ...
        const termNames = selectedTermCodes
          .map((termCode) => {
  ...
  const handleStaticFiltersChange = (
    newStaticFilters: Record<string, string[]>
  ) => {
    setStaticFilters(newStaticFilters);
    // Call the callback with merged filters
    const mergedFilters = transformFiltersForCallback(
      selectedFilters,
      newStaticFilters
    );
  ...
  const parseFrameworkData = (data: any) => {
    ...
    const parsedCategories: FilterCategory[] = data.framework.categories.map(
      (category: any) => ({
  ...
  const getDependentOptions = (
    ...
    const hasSelections = dependencies.some(
      (depCode) =>
    ...
    dependencies.forEach((dependencyCode) => {
      ...
      const dependencyCategory = categories.find(
        (cat) => cat.code === dependencyCode
      );
      ...
      selectedValues.forEach((selectedValue) => {
        const dependencyTerm = dependencyCategory.terms.find(
          (term) => term.code === selectedValue
        );
    ...
    // Remove duplicates
    const uniqueTerms = intersectionTerms.filter(
      (term, index, self) =>
        self.findIndex((t) => t.identifier === term.identifier) === index
    );
  ...
  const handleFilterChange = (
    ...
    setSelectedFilters((prev) => {
      ...
      if (checked) {
        ...
        newFilters[categoryCode] = newFilters[categoryCode].filter(
          (code) => code !== termCode
        );
      }
  ...
  const clearDependentFilters = (
    ...
    const categoriesSortedByIndex = categories
      .slice()
      .sort((a, b) => a.index - b.index);
    const changedCategoryIndex =
      categories.find((cat) => cat.code === changedCategoryCode)?.index || 0;
      ...
      if (category.index > changedCategoryIndex) {
        ...
        if (categoryDependencies) {
          ...
          const isIndirectlyAffected = categoryDependencies.some((depCode) => {
            ...
            return (
              depDependencies && depDependencies.includes(changedCategoryCode)
            );
          });
                ...
                <Box key={term.identifier} sx={{ width: '100%' }}>
                  <FormControlLabel
                    control={
                      <Checkbox
                        checked={
                          selectedFilters[category.code]?.includes(term.code) ||
                          false
                        }
                        onChange={(e) =>
                          handleFilterChange(
