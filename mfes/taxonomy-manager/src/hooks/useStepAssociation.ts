...
export const useStepAssociation = () => {
  const { categories, framework } = useFrameworkFormStore();
  ...
  const handleBatchSaveAssociations = async () => {
    ...
    try {
      ...
      // If successful, fetch updated data after a delay instead of optimistic updates
      if (results.length > 0 && results.some((r) => r.result)) {
        // Clear working associations and checked terms immediately
        ...
        // Fetch updated framework data after a delay
        setTimeout(async () => {
          try {
  ...
  const handleConfirmDeleteAssociation = async () => {
    ...
    try {
      ...
      // Fetch updated framework data after a delay instead of optimistic updates
      setTimeout(async () => {
        try {
          const { fetchAndUpdateFramework } = useFrameworkFormStore.getState();
          await fetchAndUpdateFramework();
        } catch (error) {
          console.error('Failed to refresh framework data:', error);
        }
  ...
  // Reset all form state for clean slate
  const resetFormState = () => {
    // Reset to first available category and term
    const firstCategory = categoriesWithTerms[0];
    if (firstCategory) {
      setSelectedCategoryCode(firstCategory.code);
      const firstTerm = firstCategory.terms?.[0];
      if (firstTerm) {
        setSelectedTermCode(firstTerm.code);
      } else {
        setSelectedTermCode('');
      }
    } else {
    ...
    // Reset available category selection
    const firstAvailable = categoriesWithTerms.find(
      (c) => c.code !== firstCategory?.code
    );
    if (firstAvailable) {
      setSelectedAvailableCategoryCode(firstAvailable.code);
    } else {
      setSelectedAvailableCategoryCode('');
    }
