...
const EntryContent: React.FC<EntryContentProps> = ({
    ...
    const fetchQuestionsList = async () => {
      try {
        if (observationId && entityId) {
          ...
          setSubmittedByName(
            combinedData?.assessment?.submissions?.OB?.submittedByName || ''
          );
          setSubmittedBy(
            combinedData?.assessment?.submissions?.OB?.submittedBy || ''
          );
          setSubmissionDate(
            combinedData?.assessment?.submissions?.OB?.submissionDate || ''
          );
        }
      } catch (error) {
  ...
  const formatDate = (dateString: string) => {
    ...
    return `Submitted on ${date.toLocaleDateString('en-GB', {
      day: '2-digit',
      month: 'short',
      year: 'numeric',
        ...
        {questionResponse?.map((pair: any, index: number) => (
          <Box key={index} sx={{ marginTop: '20px' }}>
            ...
            <Box sx={{ marginTop: '8px' }}>
              {pair?.question === 'Participant Name' &&
                      ...
                      {participant?.name && (
                        ...
                        >
                          {participant?.name
                            ...
                            .charAt(0)
                            ?.toUpperCase() +
                            participant?.name?.toString().slice(1)}
                        </Typography>
                      )}
                      <Typography
                        sx={{
